...
- **待办面板**：聚合三个需要用户出手的工作流门 —— 中/高复杂度「待评审」（勾选数 4）、中/高复杂度「待验收」（勾选数 6，人工确认最终验收）、「待交付确认」（勾选数 7，不分档位）
...
**设计原则**：工作单文件是唯一真相 —— 状态与待办完全由 `workorders/T-YYYYMMDD-xxx.md` 的进度清单推导，插件不做任何记账。唯一写入：点「✓ 通过 / ✓ 验收通过 / ✓ 确认交付」时由插件勾选工作单对应项（评审 / 最终验收 / 交付），使待办立即消失。更新机制：30 秒轮询 + 首次失败 3 秒快速重试 + 页面重新可见立即刷新 + 监听 AI 对工作单的写操作立即重扫 + 手动「🔄 更新」。
