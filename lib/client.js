...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        if (action === 'approve') {
          ...
          const r = await fetch('/workorders-api', {
            ...
            body: JSON.stringify({
              ...
              kind: todo.kind === 'review-pending' ? 'review' : 'accept'
            })
          });
    ...
    function statusOf(w) {
      ...
      if (c === 6) return { text: '待验收', cls: 'wo-badge-warn' };
      if (c === 7) return { text: '验收中', cls: 'wo-badge-warn' };
    ...
    function TodosPanel() {
      ...
      if (todos.length === 0) return el('div', { className: 'wo-empty' }, '暂无待办。有需要你评审/验收的操作时会出现在这里。');
