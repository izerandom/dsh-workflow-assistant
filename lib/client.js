...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let refreshToken = 0;
    // 首次加载失败（会话 cwd 未就绪 / Host 路由尚未注册）时 3 秒快速重试，避免等整轮轮询
    let retryTimer = null;
    function scheduleRetry() {
      if (retryTimer !== null) return;
      retryTimer = setTimeout(() => { retryTimer = null; refresh(); }, 3000);
    }
    ...
    async function refresh() {
      ...
      try {
        ...
        if (!res || !res.ok) { store.error = (res && res.error) || 'getState failed'; emit(); scheduleRetry(); return }
        cancelRetry();
    ...
    function TodosPanel() {
      ...
      if (todos.length === 0) return el('div', { className: 'wo-empty' }, '暂无待办。有需要你评审、验收或交付确认的操作时会出现在这里。');
    ...
    function apply(ctx) {
      ...
      ctx.timer.interval(() => refresh(), 30000);
      ctx.effect(() => () => cancelRetry());
      ctx.effect(() => {
