...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function approveText(todo) {
      ...
      if (todo.kind === 'deliver-pending') return '✅ 工作单「' + name + '」交付确认通过，工作单完成。';
      return '✅ 工作单「' + name + '」验收通过，请收尾并交付。';
    }
    function rejectText(todo) {
      const name = worksheetTitle(todo.worksheet);
      if (todo.kind === 'review-pending') return '✗ 工作单「' + name + '」评审未通过，请根据我的意见修改工作单后重新提交评审：';
      if (todo.kind === 'deliver-pending') return '✗ 工作单「' + name + '」暂不交付，请根据我的意见处理后重新提交交付：';
    ...
    async function actTodo(todo, action) {
      ...
      try {
        if (action === 'approve') {
          ...
          // ② 通过对话框发送确认文案（静态模块可直接调 conversation.send）
    ...
    function TodoItem(props) {
      ...
      const actions = (todo.kind === 'review-pending')
        ...
        : (todo.kind === 'deliver-pending')
          ? [
              el('button', { key: 'db', className: 'wo-btn wo-btn-danger', disabled: busy, onClick: () => actTodo(todo, 'reject') }, '✗ 暂不交付'),
