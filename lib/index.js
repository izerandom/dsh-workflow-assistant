...
// 提供：
//   GET  /workorders-viewer?file=xxx   → HTML 查看器（动态生成）
...
function todosFor(state) {
  ...
  if (c >= 6 && c < 8) {
    todos.push({ key: 'accept:' + state.file, kind: 'accept-pending', worksheet: state.file, title: '工作单待最终验收', note: '实现已完成（勾选数 ' + c + '/8），请按验收标准核对并给出验收结果。' })
  }
  return todos
}
...
function renderBlocks(blocks){return blocks.map(function(b){switch(b.t){case 'h2':return '<h2>'+esc(b.text)+'</h2>';case 'h3':return '<h3 style="font-size:14.5px;margin:18px 0 6px;color:#2563eb">'+esc(b.text)+'</h3>';case 'p':return '<p>'+esc(b.text)+'</p>';case 'quote':return '<blockquote>'+esc(b.text)+'</blockquote>';case 'ul':return '<ul>'+b.items.map(function(x){return '<li>'+esc(x)+'</li>';}).join('')+'</ul>';case 'checklist':return '<ul class="checks">'+b.items.map(function(x){return '<li class="'+(x.checked?'on':'')+'"><span class="box">'+(x.checked?'✓':'')+'</span>'+esc(x.text)+'</li>';}).join('')+'</ul>';case 'table':var head=b.rows[0]||[];var rows=b.rows.slice(1);var nwh=[];for(var hi=0;hi<head.length;hi++){if(head[hi]==='序号'||head[hi]==='所属切片')nwh.push(hi);}var nwb=[];for(var ni=0;ni<head.length;ni++){if(head[ni]==='依赖'||head[ni]==='状态'||head[ni]==='所属切片')nwb.push(ni);}return '<table><thead><tr>'+head.map(function(h,hi){return '<th'+(nwh.indexOf(hi)>=0?' class="nowrap"':'')+'>'+esc(h)+'</th>';}).join('')+'</tr></thead><tbody>'+rows.map(function(r){return '<tr>'+r.map(function(c,i){return '<td'+(nwb.indexOf(i)>=0?' class="nowrap"':'')+'>'+sceneTag(c)+'</td>';}).join('')+'</tr>';}).join('')+'</tbody></table>';default:return '';}}).join('');}
...
export function apply(ctx) {
  ...
  if (webServer && typeof webServer.register === 'function') {
    ...
    // 数据 API：?sessionId=xxx → 工作单状态 JSON
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/workorders-api',
      handler: async (req, res) => {
